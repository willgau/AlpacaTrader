...
awaitable<nlohmann::json> Portfolio::alpaca_get_account(
    ...
    auto results = co_await resolver.async_resolve(
        host, port, asio::redirect_error(use_awaitable, ec));
 ...
 asio::awaitable<void> Portfolio::poll_account_forever(
    ...
    std::string port
     ...
     tls_ctx.set_verify_callback(ssl::host_name_verification("paper-api.alpaca.markets"));
    ...
    for (;;) {
        try {
            auto account = co_await alpaca_get_account(host, port, tls_ctx);
